...
const signupSchema = yup.object().shape({
  ...
  confirmPassword: yup
    .string()
    .oneOf([yup.ref("password"), undefined], "Passwords must match") // Fix for password validation
...
const SignUpScreen = ({ navigation }: { navigation: any }) => {
  ...
  const {
    control,
    handleSubmit, // Corrected typo here
    formState: { errors },
  } = useForm({
  ...
  const onSubmit = async (data: any) => {
    setLoading(true);
    setFirebaseError(null); // Reset error state before signup
